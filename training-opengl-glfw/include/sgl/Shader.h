...
struct Shader
{
    Shader(const std::string& fileName, GLenum shaderType)
    {
        ...
        glShaderSource(m_shader, 1, &shaderSourceStrings, nullptr); //&shaderSourceStringLengths);
    ...
    Shader(Shader&xShader) = delete;
    Shader(Shader&& xShader) {
        this->m_shader = xShader.m_shader;
        xShader.m_shader = 0;
    }
